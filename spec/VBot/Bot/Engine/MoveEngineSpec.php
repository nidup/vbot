...
class MoveEngineSpec extends ObjectBehavior
{
    ...
    function it_chooses_to_stay_when_the_hero_has_no_path_to_the_target(Game $game, Hero $hero, DestinationInterface $target)
    {
        $game->getHero()->willReturn($hero);
        $hero->getTarget()->willReturn($target);
        $game->getShortestPath($hero, $target)->willReturn([]);
        $hero->setDirection('Stay')->shouldBeCalled();
        $this->process($game);
    }

    function it_chooses_to_move_when_the_hero_has_a_path_to_the_target(Game $game, Hero $hero, DestinationInterface $target)
    {
        $game->getHero()->willReturn($hero);
        $hero->getTarget()->willReturn($target);
        $firstNode = new Node(3, 1);
        $path = [null, $firstNode];
        $game->getShortestPath($hero, $target)->willReturn($path);
